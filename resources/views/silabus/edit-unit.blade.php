...
@section('content')
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header">
            <h5 class="mb-0">Edit Detail Silabus Kompetensi</h5>
        </div>
        <div class="card-body">
            <form
                action="{{ route('silabus.update.unit', ['document_id' => $document_id, 'unit_kompetensi_id' => $unit_kompetensi_id]) }}"
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" name="waktu_pelatihan"
                                    value="{{ old('waktu_pelatihan', $silabus->waktu_pelatihan) }}"
                                    placeholder="Contoh: 5 JP @ 45 menit">
                            </div>
                            <div class="col-md-4">
                                <label class="mb-2">Metode Pelatihan</label>
                                <input type="text" class="form-control" name="metode_pelatihan"
                                    value="{{ old('metode_pelatihan', $silabus->metode_pelatihan) }}"
                                    placeholder="Contoh: Luring/Daring/Bauran">
                            </div>
                            <div class="col-md-4">
                                <label class="mb-2">Capaian Unit Kompetensi</label>
                                <textarea class="form-control" name="capaian_unit_kompetensi" rows="3"
                                    placeholder="Setelah mengikuti pelatihan ini, peserta diharapkan mampu...">{{ old('capaian_unit_kompetensi', $silabus->capaian_unit_kompetensi) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                @foreach ($element as $item)
                    @php
                        ...
                        // Get the existing detail for this element
                        $silabusDetail = $silabusDetailMap[$item->id] ?? null;
                    @endphp
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <textarea class="form-control editor" name="indikator_{{ $item->id }}" rows="3"
                                        placeholder="Pisahkan dengan titik koma (;)">{{ old('indikator_' . $item->id, $silabusDetail->indikator_unjuk_kerja ?? '') }}</textarea>
                                </div>

                                <div class="col-md-4 mb-3">
                                    <label class="mb-2">Pengetahuan</label>
                                    <textarea class="form-control editor" name="pengetahuan_{{ $item->id }}" rows="3"
                                        placeholder="Pisahkan dengan titik koma (;)">{{ old('pengetahuan_' . $item->id, $silabusDetail->pengetahuan ?? '') }}</textarea>
                                </div>

                                <div class="col-md-4 mb-3">
                                    <label class="mb-2">Keterampilan & Sikap</label>
                                    <textarea class="form-control editor" name="keterampilan_sikap_{{ $item->id }}" rows="3"
                                        placeholder="Pisahkan dengan titik koma (;)">{{ old('keterampilan_sikap_' . $item->id, $silabusDetail->keterampilan_sikap ?? '') }}</textarea>
                                </div>

                                <div class="col-md-2">
                                    <label class="mb-2">Durasi (Menit)</label>
                                    <input type="text" class="form-control" name="durasi_{{ $item->id }}"
                                        value="{{ old('durasi_' . $item->id, $silabusDetail->durasi ?? '') }}"
                                        placeholder="75">
                                </div>
